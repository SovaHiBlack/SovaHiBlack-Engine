...
void CAI_Stalker::g_fireParams(const CHudItem* pHudItem, fVector3& P, fVector3& D)
{
...
void CAI_Stalker::HitSignal				(float amount, fVector3& vLocalDir, CObject* who, s16 element)
{
	if (getDestroy())
		return;
...
void CAI_Stalker::can_kill_entity		(const fVector3& position, const fVector3& direction, float distance, collide::rq_results& rq_storage)
{
...
void CAI_Stalker::can_kill_entity_from	(const fVector3& position, fVector3 direction, float distance)
{
	m_pick_distance			= 0.f;
	rq_storage.r_clear		();
...
void CAI_Stalker::throw_target				(const fVector3& position)
{
