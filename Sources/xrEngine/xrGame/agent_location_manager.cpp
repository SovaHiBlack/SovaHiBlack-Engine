...
const f32 MIN_SUITABLE_ENEMY_DISTANCE = 3.0f;//10.f;
...
struct CDangerLocationPredicate {
	...
	IC			CDangerLocationPredicate	(const fVector3& position)
	{
		m_position		= position;
	}
...
IC	CAgentLocationManager::CDangerLocationPtr CAgentLocationManager::location	(const fVector3& position)
{
...
f32 CAgentLocationManager::danger		(const CCoverPoint *cover, CAI_Stalker *member) const
{
	...
	for ( ; I != E; ++I) {
		...
		f32					distance = 1.0f + (*I)->position().distance_to(cover->position());
		if (distance > (*I)->m_radius)
			continue;

		result *= f32(Device.dwTimeGlobal - (*I)->m_level_time) / f32((*I)->m_interval);
	}
