...
void CALifeSwitchManager::try_switch_online	(CSE_ALifeDynamicObject	*I)
{
	...
	if (0xffff != I->ID_Parent) {
		...
		if (psAI_Flags.test(aiALife)) {
			...
			if (l_tpALifeCreatureAbstract && (l_tpALifeCreatureAbstract->fHealth < EPS_3))
			...
			VERIFY2				(!l_tpALifeCreatureAbstract || (l_tpALifeCreatureAbstract->fHealth >= EPS_3),"Parent online, item offline...");
...
void CALifeSwitchManager::try_switch_offline(CSE_ALifeDynamicObject	*I)
{
	...
	if (0xffff != I->ID_Parent) {
		...
		VERIFY2				(!smart_cast<CSE_ALifeCreatureAbstract*>(objects().object(I->ID_Parent)) || (smart_cast<CSE_ALifeCreatureAbstract*>(objects().object(I->ID_Parent))->fHealth >= EPS_3),"Parent offline, item online...");
