...
void CCarDamageParticles::Play1(CCar* car)
{
	if(*m_car_damage_particles1)
	{
		...
		for(;e!=i;++i) car->StartParticles(m_car_damage_particles1,*i, fVector3().set(0.0f,1.0f,0.0f),car->ID());
	}
}
...
void CCarDamageParticles::Play2(CCar* car)
{
	...
	if(*m_car_damage_particles2)
	{
		...
		for(;e!=i;++i) car->StartParticles(m_car_damage_particles2,*i, fVector3().set(0.0f,1.0f,0.0f),car->ID());
	}
}
...
{
	VERIFY(!ph_world->Processing());
	if(*m_wheels_damage_particles1)car->StartParticles(m_wheels_damage_particles1,bone_id, fVector3().set(0.0f,1.0f,0.0f),car->ID());
}

void CCarDamageParticles::PlayWheel2(CCar*car,u16 bone_id)
{
	VERIFY(!ph_world->Processing());
	if(*m_wheels_damage_particles2)car->StartParticles(m_wheels_damage_particles2,bone_id, fVector3().set(0.0f,1.0f,0.0f),car->ID());
}
