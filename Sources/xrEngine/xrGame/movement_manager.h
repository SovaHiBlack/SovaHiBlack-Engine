...
public:
	...
	IC		const CBoneRotation &body_orientation() const;
			void	update_path				();
	virtual	void	move_along_path			(CPHMovementControl *movement_control, fVector3& dest_position, float time_delta);
	...
	template <typename T>
	IC		bool	accessible				(T position_or_vertex_id, float radius = EPS_3) const;
