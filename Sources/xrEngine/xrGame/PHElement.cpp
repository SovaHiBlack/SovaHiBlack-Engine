...
void CPHElement::calculate_it_data(const fVector3& mc,float mas)
{
...
void CPHElement::calc_it_fract_data_use_density(const fVector3& mc,float density)
{
...
void		CPHElement::	setDensityMC		(float M,const fVector3& mass_center)
{
...
void		CPHElement::	setMassMC		(float M,const fVector3& mass_center)
{
	m_mass_center.set(mass_center);
	calc_volume_data();
...
void CPHElement::Activate(const fMatrix4x4& m0,float dt01,const fMatrix4x4& m2,bool disable)
{
	fVector3 lvel;
	fVector3 avel;
...
void	CPHElement::applyImpulseVsMC(const fVector3& pos,const fVector3& dir, float val)
{
...
void	CPHElement::applyImpulseVsGF(const fVector3& pos,const fVector3& dir, float val)
{
...
IC bool put_in_range(fVector3& v, float range )
{
	VERIFY( range > EPS_S );
...
void	CPHElement::applyForce(const fVector3& dir, float val)															//aux
{
...
void	CPHElement::applyImpulse(const fVector3& dir, float val)//aux
{
...
void CPHElement::add_Mass(const SBoneShape& shape,const fMatrix4x4& offset,const fVector3& mass_center,float mass,CPHFracture* fracture)
{
	dMass m;
	dMatrix3 DMatx;
	switch(shape.type) 
	...
	case SBoneShape::stSphere	:
		{
			shape.sphere;
			dMassSetSphere(&m,1.0f,shape.sphere.R);
			dMassAdjust(&m,mass);
	...
	case SBoneShape::stCylinder :
		{
			...
			dMassSetCylinder(&m,1.0f,2,shape.cylinder.m_radius,shape.cylinder.m_height);
			dMassAdjust(&m,mass);
			dMatrix3 DMatx;
			fMatrix3x3 m33;
	...
	new_mc.mul(1.0f/(mass+m_mass.mass));
	mc.sub(new_mc);
...
void CPHElement::calculate_it_data_use_density(const fVector3& mc,float density)
{
	dMassSetZero(&m_mass);
...
void CPHElement::get_Extensions(const fVector3& axis,float center_prg,float& lo_ext, float& hi_ext)
{
...
void CPHElement::ReAdjustMassPositions(const fMatrix4x4& shift_pivot,float density)
{
...
void CPHElement::ReInitDynamics(const fMatrix4x4& shift_pivot,float density)
{
