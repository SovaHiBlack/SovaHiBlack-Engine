...
IC	fVector3 construct_position		(u32 level_vertex_id, float x, float z)
{
	return							(fVector3().set(x,ai().level_graph().vertex_plane_y(level_vertex_id,x,z),z));
}

IC	bool CSpaceRestrictionBase_inside	(CSpaceRestrictionBase *self, const fVector3& position, const float &radius)
{
	fSphere							sphere;
	sphere.P						= position;
	sphere.R						= radius;
...
bool CSpaceRestrictionBase::inside	(u32 level_vertex_id, bool partially_inside, float radius)
{
	...
	float							offset = ai().level_graph().header().cell_size()*.5f - EPS_3;
	if (partially_inside)
		return						(
