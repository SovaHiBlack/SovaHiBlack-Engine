...
public:
	...
	{
		if (!fsimilar(arg[0],W.arg[0], EPS_3))	return FALSE;
		if (!fsimilar(arg[1],W.arg[1], EPS_3))	return FALSE;
		if (fis_zero(arg[1], EPS_3))				return TRUE;
		if (F	!=	W.F)						return FALSE;
		if (!fsimilar(arg[2],W.arg[2], EPS_3))	return FALSE;
		if (!fsimilar(arg[3],W.arg[3], EPS_3))	return FALSE;
		return TRUE;
	}
};
