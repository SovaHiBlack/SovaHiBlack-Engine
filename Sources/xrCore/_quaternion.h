...
public:
	using TYPE = T;
	using Self = _quaternion<TYPE>;
	using SelfRef = Self&;
	using SelfCRef = const Self&;

private:
	IC TYPE _asin_(TYPE x)
	{
		const TYPE c1 = 0.892399f;
		const TYPE c3 = 1.693204f;
		const TYPE c5 = -3.853735f;
		const TYPE c7 = 2.838933f;

		const TYPE x2 = x * x;
		const TYPE d = x * (c1 + x2 * (c3 + x2 * (c5 + x2 * c7)));

		return d;
	}
	IC TYPE _acos_(TYPE x)
	{
...
public:
	...
	IC SelfRef set(TYPE W, TYPE X, TYPE Y, TYPE Z)	// don't normalize
	{
		x = X;
		y = Y;
		z = Z;
	...
	IC SelfRef set(const _matrix4x4<TYPE>& m);
...
*/
	...
	// makes unit rotation
	IC SelfRef rotationYawPitchRoll(TYPE _x, TYPE _y, TYPE _z)
	{
		TYPE fSinYaw = _sin(_x * 0.5f);
		TYPE fCosYaw = _cos(_x * 0.5f);
		TYPE fSinPitch = _sin(_y * 0.5f);
		TYPE fCosPitch = _cos(_y * 0.5f);
		TYPE fSinRoll = _sin(_z * 0.5f);
		TYPE fCosRoll = _cos(_z * 0.5f);
	...
	IC SelfRef rotation(fVector3& axis, TYPE angle)
	{
		TYPE sinTheta;
		w = _cos(angle * 0.5f);
	...
	IC BOOL get_axis_angle(fVector3& axis, TYPE& angle)
	{
		TYPE OneOverSinTheta;
		TYPE HalfTheta = acosf(w);
	...
	ICF SelfRef slerp(SelfCRef Q0, SelfCRef Q1, TYPE tm)
	{
		TYPE Scale0;
		TYPE Scale1;
		TYPE sign;
		...
		TYPE cosom = (Q0.w * Q1.w) + (Q0.x * Q1.x) + (Q0.y * Q1.y) + (Q0.z * Q1.z);
		if (cosom < 0)
		{
			cosom = -cosom;
			sign = -1.0f;
		...
		if ((1.0f - cosom) > EPS)
		{
			TYPE omega = _acos_(cosom);
			TYPE i_sinom = 1.0f / _sin(omega);
			TYPE t_omega = tm * omega;
	...
	IC BOOL cmp(SelfCRef Q, TYPE Tolerance = 0.0001f)
	{
	...
	{
		TYPE n = Q.x * Q.x + Q.y * Q.y + Q.z * Q.z;
		TYPE r = _sqrt(n);
		TYPE t = (r > EPS_S) ? atan2f(r, Q.w) / r : 0.0f;
		x = t * Q.x;
		y = t * Q.y;
		z = t * Q.z;
		w = 0.5f * _log(n + Q.w * Q.w);
		return *this;
	}
	IC SelfRef exp(SelfCRef Q)
	{
		TYPE r = _sqrt(Q.x * Q.x + Q.y * Q.y + Q.z * Q.z);
		TYPE et = expf(Q.w);
		TYPE s = (r >= EPS_S) ? et * _sin(r) / r : 0.0f;
		x = s * Q.x;
		y = s * Q.y;
		z = s * Q.z;
		w = et * _cos(r);
...
//using dQuaternion = _quaternion<f64>;
