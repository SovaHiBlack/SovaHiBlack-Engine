...
public:
	...
	IC SelfRef rotation(const _quaternion<TYPE>& Q);
	ICF SelfRef mk_xform(const _quaternion<TYPE>& Q, const Tvector& V);

	// Multiply RES = A[4x4]*B[4x4] (WITH projection)
	ICF SelfRef mul(SelfCRef A, SelfCRef B)
	{
	...
	ICF SelfRef mul_43(SelfCRef A, SelfCRef B)
	{
		VERIFY((this != &A) && (this != &B));
	...
	IC SelfRef mulA_44(SelfCRef A)			// mul after 
	{
		Self B;
		B.set(*this);
		mul(A, B);
	...
	IC SelfRef mulB_44(SelfCRef B)			// mul before
	{
		Self A;
		A.set(*this);
		mul(A, B);
	...
	ICF SelfRef mulA_43(SelfCRef A)			// mul after (no projection)
	{
		Self B;
		B.set(*this);
		mul_43(A, B);
	...
	ICF SelfRef mulB_43(SelfCRef B)			// mul before (no projection)
	{
		Self A;
		A.set(*this);
		mul_43(A, B);
	...
	IC SelfRef translate(TYPE _x, TYPE _y, TYPE _z) // setup translation matrix
	{
		identity( );
		c.set(_x, _y, _z);
		return *this;
	...
	IC SelfRef translate_over(TYPE _x, TYPE _y, TYPE _z) // modify only translation
	{
		c.set(_x, _y, _z);
		return *this;
	}
	...
	IC SelfRef scale(TYPE x, TYPE y, TYPE z)	// setup scale matrix
	{
		identity( );
		m[0][0] = x;
		m[1][1] = y;
	...
	IC SelfRef rotateX(TYPE Angle)				// rotation about X axis
	{
	...
	IC SelfRef rotateY(TYPE Angle)				// rotation about Y axis
	{
		TYPE cosa = _cos(Angle);
		TYPE sina = _sin(Angle);
		i.set(cosa, 0, -sina);
	...
	IC SelfRef rotateZ(TYPE Angle)				// rotation about Z axis
	{
		TYPE cosa = _cos(Angle);
		TYPE sina = _sin(Angle);
		i.set(cosa, sina, 0);
	...
	IC SelfRef rotation(const Tvector& axis, TYPE Angle)
	{
	...
	IC SelfRef mul(SelfCRef A, TYPE v)
	{
	...
	IC SelfRef div(SelfCRef A, TYPE v)
	{
		VERIFY(_abs(v) > 0.000001f);
		return mul(A, 1.0f / v);
	}
	IC SelfRef div(TYPE v)
	{
		VERIFY(_abs(v) > 0.000001f);
		return mul(1.0f / v);
	}
	// fov
	IC SelfRef build_projection(TYPE fFOV, TYPE fAspect, TYPE fNearPlane, TYPE fFarPlane)
	{
		return build_projection_HAT(tanf(fFOV / 2.0f), fAspect, fNearPlane, fFarPlane);
	}
	// half_fov-angle-tangent
	IC SelfRef build_projection_HAT(TYPE HAT, TYPE fAspect, TYPE fNearPlane, TYPE fFarPlane)
	{
		...
		TYPE w = fAspect * cot;
		TYPE h = TYPE(1) * cot;
		TYPE Q = fFarPlane / (fFarPlane - fNearPlane);
	...
	IC SelfRef build_projection_ortho(TYPE w, TYPE h, TYPE zn, TYPE zf)
	{
		_11 = TYPE(2) / w;	_12 = 0;			_13 = 0;					_14 = 0;
		_21 = 0;			_22 = TYPE(2) / h;	_23 = 0;					_24 = 0;
		_31 = 0;			_32 = 0;			_33 = TYPE(1) / (zf - zn);	_34 = 0;
		_41 = 0;			_42 = 0;			_43 = zn / (zn - zf);		_44 = TYPE(1);
		return *this;
	}
	IC SelfRef build_camera(const Tvector& vFrom, const Tvector& vAt, const Tvector& vWorldUp)
	{
		...
		TYPE fDotProduct = vWorldUp.dotproduct(vView);
	...
	IC SelfRef inertion(SelfCRef mat, TYPE v)
	{
		TYPE iv = 1.0f - v;
		for (s32 i = 0; i < 4; i++)
		{
	...
	ICF SelfRef setHPB(TYPE h, TYPE p, TYPE b)
	{
		TYPE _ch;
		TYPE _cp;
		TYPE _cb;
	...
	IC SelfRef setXYZ(TYPE x, TYPE y, TYPE z)
	{
		return setHPB(y, x, z);
	}
	...
	IC SelfRef setXYZi(TYPE x, TYPE y, TYPE z)
	{
		return setHPB(-y, -x, -z);
	}
...
//
	IC void getHPB(TYPE& h, TYPE& p, TYPE& b) const
	{
		TYPE cy = _sqrt(j.y * j.y + i.y * i.y);
		if (cy > 16.0f * type_epsilon(TYPE))
		{
			h = (TYPE)-atan2(k.x, k.z);
			p = (TYPE)-atan2(-k.y, cy);
			b = (TYPE)-atan2(i.y, j.y);
		}
		else
		{
			h = (TYPE)-atan2(-i.z, i.x);
			p = (TYPE)-atan2(-k.y, cy);
			b = 0;
		}
	}
	...
	IC void getXYZ(TYPE& x, TYPE& y, TYPE& z) const
	{
		getHPB(y, x, z);
	}
	...
	IC void getXYZi(TYPE& x, TYPE& y, TYPE& z) const
	{
		getHPB(y, x, z);
		x *= -1.0f;
		y *= -1.0f;
